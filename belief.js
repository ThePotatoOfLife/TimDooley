...
function recordLink(id, label) {
  return `<a class="belief-record-link" href="belief.html?id=${encodeURIComponent(id)}">${esc(label)}</a>`;
}
...
function renderRecord(id, political, religious, potatoism, foundations) {
  ...
  if (!pe && !re && id !== 'potatoism') {
    $('app').innerHTML = `<section class="belief-error"><span class="belief-type">IDEAS</span><h1>Belief not found</h1><p>The requested identifier <code>${esc(id)}</code> is not currently in the atlas.</p><a class="belief-back" href="belief.html">← Browse Ideas</a></section>`;
    return;
  }

  if (pe) {
    document.title = `${pretty(id)} — Political Belief`;
    $('app').innerHTML = `<section class="belief-record"><span class="belief-type">Political ideology</span><h1>${esc(pretty(id))}</h1><p class="record-lead">${esc(pe[1])}</p><h2>Analytical coordinates</h2><div class="coords"><span>Economic <b>${esc(pe[2])}</b></span><span>Authority <b>${esc(pe[3])}</b></span><span>Quadrant <b>${esc(pe[4])}</b></span></div><p class="note">Coordinates are visualization approximations, not a complete definition of an ideology or its adherents.</p><a class="belief-back" href="belief.html">← All ideas</a></section>`;
    return;
  }

  if (id === 'potatoism') {
    document.title = 'Potatoism — The Potato of Life';
    $('app').innerHTML = `<section class="belief-record"><span class="belief-type">Project-defined contemporary religious / mythic system</span><h1>Potatoism</h1><p class="record-lead">${esc(potatoism.epistemic_note || '')}</p><h2>Birth / seed</h2><div class="mini"><strong>${esc(potatoism.birth?.date)}</strong><br>${esc(potatoism.birth?.meaning)}</div><h2>Growing timeline</h2><div class="timeline">${(potatoism.timeline || []).map(x => `<article><strong>${esc(x.period || x.stage)}</strong><p>${esc(x.stage || '')}</p><div>${esc(x.description || '')}</div></article>`).join('')}</div><h2>Symbolic architecture</h2><div class="belief-grid">${Object.entries(potatoism.core_architecture || {}).map(([k,v]) => `<div class="mini"><strong>${esc(k)}</strong><p>${esc(Array.isArray(v) ? v.join(' · ') : JSON.stringify(v))}</p></div>`).join('')}</div><p class="note">The birth marker is treated as the seed of the timeline, not as the date on which the finished architecture existed.</p><a class="belief-back" href="belief.html">← All ideas</a></section>`;
    return;
  }
...
async function init() {
  ...
  try {
    const [political, religious, potatoism, foundations] = await Promise.all([
    ...
    if (id) renderRecord(id, political, religious, potatoism, foundations);
    else renderDirectory(political, religious);
  } catch (error) {
    $('app').innerHTML = `<section class="belief-error"><span class="belief-type">IDEAS</span><h1>Belief data unavailable</h1><p>A belief data layer could not be loaded: ${esc(error.message)}</p></section>`;
  }
}
init();
