...
- [ ] Audit the 38 nested Room pages for shell consistency. **Phase 1 complete:** 20 shell-only older Rooms now use `app/room-interior.css`, with validator protection against reintroducing the duplicated `.inner-home/.inner-center/.adj-grid` block. Continue separating unique selectors from the remaining richer Room pages before migrating them.
