...
- [~] **NAV-BUG-002 · “Read” means read:** architecture audit now inventories visible Read/Open/Enter anchors and warns when they land on registered hub/explorer shells rather than direct substance. Remaining: review flagged cases and relabel or retarget only genuine promise mismatches.
...
- [~] **READ-004 · Dead-end reader audit:** registered routing now treats Home-only continuation as a dead end, and the audit also checks a governed standalone-public set outside `public-surfaces.json`. The standalone TTS tool now has an explicit `← Site` return instead of relying on browser history or an injected dock. Remaining: review any additional standalone/public exceptions flagged by the audit.
- [~] **READ-005 · Vague-link language:** architecture audit now flags bare standalone More / Deep / Explore / Context / Archive labels and records their targets. Remaining: review flagged cases and replace only genuinely ambiguous labels with destination intent.
...
- [~] **SITE-ARCH-021 · Route graph health artifact:** active-surface metrics, authored first-substance density, registered outdegree, repeated destinations, reader-job presence, high-overlap pairs, registered dead ends and two-way cycles are now emitted to `.quality-logs/site-architecture-audit.json` and run in the content quality group. Authored anchor counts are separated from runtime/script href literals to avoid false density alarms. Remaining: extend into the complete built-site graph for unregistered orphan/dead-end and longer-cycle detection.
...
- [~] **SITE-ARCH-032 · House / Rooms boundary sharpening:** Rooms already hands House-owned topology back to House; House now also removes Inhabitants/Paths/global Home from its first navigation row and keeps only Rooms + Living Axis as structural continuations. Remaining: audit deeper House sections for Room-directory behavior that should hand back to Rooms.
...
- [~] **SITE-ARCH-034 · Home / House / Rooms three-layer test:** progression is now materially sharper: Rooms hands House-owned topology back to House, and Home’s four-card architecture spine is compressed to one orientation sentence. Continue auditing House for directory behavior and Home for later structural duplication before closing.
