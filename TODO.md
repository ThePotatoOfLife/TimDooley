...
- [~] **READ-004 · Dead-end reader audit:** registered routing now treats Home-only continuation as a dead end, and the audit also scans authored public HTML outside `public-surfaces.json`. The standalone TTS tool now has an explicit `← Site` return instead of becoming an island. Remaining: repair any additional authored/public exceptions flagged by the broader scan.
