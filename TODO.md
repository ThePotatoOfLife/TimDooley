...
- [x] **CLEAN-004 · Major-reader inline CSS extraction:** Home (~22 KB), House (~21 KB), Axis (~14 KB), Rooms (~12 KB), Religion (~7.7 KB), Potato of Life (~7.5 KB) and Tim (~6.6 KB) now keep their substantial presentation in page-scoped `app/` stylesheets; reader-richness validation rejects regression back to large page-local `<style>` ownership. Leave genuinely small page-local composition alone unless it becomes reusable.
