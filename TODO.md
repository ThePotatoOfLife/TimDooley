...
- [x] **CLEAN-004 · Major-reader inline CSS extraction:** Home (~22 KB), House (~21 KB), Axis (~14 KB) and Rooms (~12 KB) have moved their former inline CSS verbatim into `app/home.css`, `app/house.css`, `app/axis.css` and `app/rooms.css`; reader-richness validation rejects regression back to large page-local `<style>` ownership.
