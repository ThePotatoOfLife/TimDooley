...
- [ ] **SITE-ARCH-012 · Public-surface redundancy matrix:** compare Home, House, Rooms, Explore, A–Z, Paths, Questions, Timeline and major hubs for duplicated navigation blocks, duplicated intros and duplicated destination sets; merge or demote blocks when two surfaces are solving the same reader problem.
...
- [ ] **SITE-ARCH-021 · Route graph health artifact:** generate a dated machine-readable whole-site graph report with orphan pages, dead ends, high-degree hubs, short cycles, unreachable specialist surfaces, duplicate route labels and owner mismatches; keep historical snapshots lightweight and let CI compare only meaningful regressions.
