...
const spatial = fs.readFileSync(new URL('world-map/3d-spatial-overlays.js', root), 'utf8');
...
assert.ok(app.includes("routeCoordinates=geoKernel.shortestWrappedLine"), 'symbolic interior links must use wrap-safe world-copy geometry');
assert.ok(!spatial.includes("shortestWrappedLine("), 'stored spatial-overlay line geometry must not be silently coerced into schematic chord geometry');
assert.ok(
