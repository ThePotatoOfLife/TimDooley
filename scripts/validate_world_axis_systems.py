...
ORDINARY_ROLES = {"primary", "secondary", "bridge", "shared"}
...
def validate_axis_source(codes: set[str]) -> dict:
    ...
    for code, profile in profiles.items():
        ...
        for item in orientations:
            ...
            assert axis in AXES, f"{code}: invalid axis {axis!r}"
            assert role in ROLES, f"{code}: invalid role {role!r}"
            assert confidence in CONFIDENCE, f"{code}: invalid confidence {confidence!r}"
...
def validate_groups() -> dict:
    ...
    for group_id, (official_count, polygon_count) in REQUIRED_GROUP_COUNTS.items():
        ...
        assert str(group.get("as_of") or "").strip(), f"{group_id}: as_of required"
        assert str(group.get("source") or "").strip(), f"{group_id}: source required"
        assert str(group.get("source_url") or "").startswith("https://"), f"{group_id}: source_url required"
    return data
...
def validate_chains(codes: set[str]) -> dict:
    ...
    for chain_id, chain in chains.items():
        ...
        assert all(code in codes for code in members), f"{chain_id}: non-canonical member"
    return data


def validate_runtime(codes: set[str]) -> None:
    runtime = load_json("data/world-map-data-runtime.json")
...
def validate_registry_and_browser() -> None:
    ...
    assert "axisMembers" in compositor
    assert "axisProfile" in compositor
    assert "referenceFigures" in compositor
    assert "chainsForCountry" in compositor
    ...
    assert "projection" in world_bar
    assert "setProjection" in world_bar
    assert "mercator" in world_bar and "globe" in world_bar
    assert "__potatoAtlasProjection" in world_bar
