...
def find_link(text: str, rel: str) -> str | None:
    ...
    for tag in LINK_RE.findall(text):
        a = attrs(tag)
        rels = {part.lower() for part in a.get("rel", "").split()}
        if wanted in rels:
            return a.get("href", "")
    return None
...
def page_url(page: Path) -> str:
    ...
    if rel == Path("."):
        return BASE_URL + "/"
    return f"{BASE_URL}/{rel.as_posix().strip('/')}/"
...
def page_title(text: str, page: Path) -> str:
    ...
    rel = page.parent.relative_to(OUT)
    if rel == Path("."):
        return "The Potato of Life — Tim Dooley Archive"
...
def inject_metadata(text: str, page: Path) -> tuple[str, int]:
    ...
    twitter = {
        ...
        "twitter:description": clip(description, 200),
    }
...
def link_known_record_paths(text: str, routes: dict[str, str]) -> tuple[str, int]:
    """Turn generated owner/path labels into real internal links.

    This is intentionally limited to exact ``knowledge/`` and ``data/`` code
    labels that resolve to deployed files. Core-index owners link to their human
    record page; other deployed source owners link to the source artifact.
    """
    linked = 0
    ...
    def replace(match: re.Match[str]) -> str:
        ...
        if not raw.startswith(("knowledge/", "data/")):
        ...
        if re.search(r"<a\b[^>]*>\s*$", prefix, re.I):
            return match.group(0)
        href = routes.get(raw)
        if not href:
            href = f"{BASE_URL}/{quote(raw, safe='/._-')}"
        linked += 1
...
def sitemap_children() -> list[Path]:
    ...
    for node in root.findall(f"{{{SITEMAP_NS}}}sitemap"):
        ...
        if loc is None or not loc.text or not loc.text.startswith(BASE_URL + "/"):
            continue
...
def main() -> None:
    ...
    print(
        ...
        f"{report['pages']} HTML pages, {changed_pages} pages normalized, "
