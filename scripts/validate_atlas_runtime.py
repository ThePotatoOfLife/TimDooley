...
def main()->int:
    ...
    if runtime.get('schema_version')!='1.2.0':raise SystemExit('ATLAS RUNTIME FAILED: schema version')
    ...
    for node_id,node in nodes.items():
        ...
        for block in blocks:
            if not isinstance(block,dict) or not str(block.get('id','')).strip() or not str(block.get('title','')).strip() or not str(block.get('text','')).strip():
                raise SystemExit(f'ATLAS RUNTIME FAILED: malformed readable block for {node_id}')
