...
ERRORS=[]
# Build marker strings without embedding the audit vocabulary literally in this source file.
_TERMS=["TO"+"DO","FIX"+"ME","PLACE"+"HOLDER","T"+"BD","T"+"BA","COMING"+" SOON","UNDER"+" CONSTRUCTION"]
...
def main():
    ...
    extensions={".html",".js",".css",".json",".py",".yml",".yaml"}
    ...
    for p in ROOT.rglob("*"):
        ...
        # HTML input attributes named placeholder are interface text, not unfinished records.
        scan=re.sub(r"\bplaceholder\s*=", "", text, flags=re.I)
        for m in BAD_TERMS.finditer(scan):
            line=scan.count("\n",0,m.start())+1
            ERRORS.append(f"Unfinished-work marker in {p.relative_to(ROOT)}:{line}")
