...
def line_for(source:str, offset:int)->int: return source.count("\n",0,offset)+1

def record(kind,resource,module,operation,line,confidence="high",**details):
    return {"kind":kind,"resource":resource,"module":module,"operation":operation,"line":line,"confidence":confidence,"details":details}

def discover_modules(root:Path):
    world=root/"world-map"; return sorted(world.glob("3d-*.js")) if world.is_dir() else []

def _nearest_map_event(source:str, offset:int):
    matches=list(re.finditer(r"\bmap\.(?:on|once)\(\s*['\"]([^'\"]+)['\"]",source[:offset]))
    return matches[-1].group(1) if matches else None

def _browser_url_vars(source:str):
...
def scan_module(path:Path, repo_root:Path):
    ...
    for var in _browser_url_vars(source):
        pat=rf"\b{re.escape(var)}\.searchParams\.(?:set|delete)\(\s*['\"]([^'\"]+)['\"]"
        for m in re.finditer(pat,source): out.append(record("url_write",f"url:{m.group(1)}",module,"searchParams",line_for(source,m.start()),variable=var))
    ...
    for m in epat.finditer(source):
        ...
        if event=="styledata":
            ctx=source[max(0,m.start()-400):min(len(source),m.start()+1800)]
            out.append(record("style_restore",f"style-restore:{module}",module,"styledata",line_for(source,m.start()),guarded=("restoring" in ctx),deferred=("queueMicrotask" in ctx)))
    for m in re.finditer(r"\b(window|document)\.addEventListener\(\s*['\"]([^'\"]+)['\"]",source):
        out.append(record("dom_listener",f"dom-event:{m.group(1)}:{m.group(2)}",module,"addEventListener",line_for(source,m.start()),target=m.group(1),event=m.group(2)))
    fspat=re.compile(r"\bmap\.setFeatureState\(\s*\{[\s\S]{0,300}?\bsource\s*:\s*['\"]([^'\"]+)['\"][\s\S]{0,300}?\}\s*,\s*\{([\s\S]{0,400}?)\}\s*\)",re.M)
    ...
    rpats=(re.compile(r"\b(?:renderStack|stack)\.register\(\s*['\"]([^'\"]+)['\"]\s*,\s*['\"]([^'\"]+)['\"]"),re.compile(r"\b(?:renderStack|stack)\.register\(\s*['\"]([^'\"]+)['\"]\s*,\s*\{[\s\S]{0,250}?\bslot\s*:\s*['\"]([^'\"]+)['\"]"))
    for pat in rpats:
    ...
    for i,m in enumerate(re.finditer(r"new\s+maplibregl\.Popup\s*\(",source),1):
        event=_nearest_map_event(source,m.start()); ctx=source[max(0,m.start()-1200):min(len(source),m.end()+2200)]
        out.append(record("popup",f"popup:{module}:{i}",module,"Popup",line_for(source,m.start()),transient=event in HOVER_EVENTS,trigger_event=event,has_hover_class="atlas-hover" in ctx))
...
def contract_shared(contract,resource_id,modules):
    entry=(contract.get("shared") or {}).get(resource_id)
    return isinstance(entry,dict) and bool(entry.get("rationale")) and modules<=set(entry.get("modules") or [])
...
def analyze(records,contract,existing_modules):
    ...
    findings.sort(key=lambda x:(0 if x["severity"]=="error" else 1 if x["severity"]=="warning" else 2,x["domain"],x["code"],x["message"]))
    return findings,ownership

def build_report(records,findings,ownership):
    inv=defaultdict(list)
    for row in records:inv[row["kind"]].append(row)
    inventory={k:sorted(v,key=lambda r:(r["resource"],r["module"],r["line"])) for k,v in sorted(inv.items())}
    domains={d:0 for d in RISK_DOMAINS}
    for item in findings:domains[item["domain"]]=domains.get(item["domain"],0)+RISK_WEIGHTS.get(item["severity"],0)
    summary={"modules":len({r["module"] for r in records}),"resources":len(ownership),"errors":sum(i["severity"]=="error" for i in findings),"warnings":sum(i["severity"]=="warning" for i in findings),"notes":sum(i["severity"]=="note" for i in findings),"risk_score":min(100,sum(domains.values()))}
    actions=[{"domain":d,"risk":s} for d,s in sorted(domains.items(),key=lambda p:(-p[1],p[0])) if s>0][:5]
...
def main(argv=None):
    ...
    try:
        ...
        print(f"WORLD MAP ARCHITECTURE AUDIT FAILED TO RUN: {exc}",file=sys.stderr); return 2
