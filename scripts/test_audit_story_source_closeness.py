...
class SourceHintTests(unittest.TestCase):
    def test_unregistered_source_notes_become_hints_without_claiming_verified_mapping(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            (content / "a.html").write_text(
                ...
                '</span></details></article>', encoding="utf-8")
            ...
            (content / "c.html").write_text(
                ...
                '<span class="source-paths">The Great Book Of Potato2.odt</span></p></details></article>', encoding="utf-8")
            (content / "d.html").write_text(
                ...
                '<span class="source-paths">knowledge/timeline/developmental-genealogy.json</span></p></details></article>', encoding="utf-8")
            (content / "e.html").write_text(
                '<article class="story-entry" data-story-type="side" id="blank"></article>', encoding="utf-8")
            (content / "f.html").write_text(
                ...
                '<span class="source-paths">knowledge/unknown/source.json</span></p></details></article>', encoding="utf-8")
            ...
            self.assertEqual(
                ...
                ["blank", "mystery-hint", "conversation-recovery", "great-book-retro", "note-only-retro", "note-only-public", "public-post", "note-only-creative", "song"],
            )
    ...
    def test_source_note_vocabulary_normalizes_to_conservative_source_families(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            dump(root / "knowledge/story/story-registry.json", {"stories": []})
            dump(root / "knowledge/story/source-records.json", {"sources": []})
            ...
            content.mkdir(parents=True)
            examples = {
                "underscore-creative": "source note great_book_creative The Great Book Of Potato2.odt",
                "retro-recovery": "source note retrospective recovery knowledge/timeline/developmental-genealogy.json",
                "statement-ledger": "Source note Conversation-archaeology statement ledger for recovered context.",
                "project-biography": "Source note Later Great Book/project biography. Earliest incident documentation has not yet been recovered.",
            }
            for index, (story_id, note) in enumerate(examples.items()):
                (content / f"{index}.html").write_text(
                    f'<article class="story-entry" data-story-type="side" id="{story_id}">'
                    f'<details class="source-note"><p>{note}</p></details></article>',
                    encoding="utf-8",
                )

            records = {r["story_id"]: r for r in build_audit(root)["records"]}
            self.assertEqual(records["underscore-creative"]["hinted_source_classes"], ["great_book_literary_text"])
            self.assertEqual(records["retro-recovery"]["hinted_source_classes"], ["archaeology_summary"])
            self.assertEqual(records["statement-ledger"]["hinted_source_classes"], ["conversation_recovery", "statement_ledger"])
            self.assertEqual(records["project-biography"]["hinted_source_classes"], ["later_autobiographical_retelling"])
