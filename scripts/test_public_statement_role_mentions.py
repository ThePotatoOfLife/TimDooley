...
def main() -> int:
    tests = [
        test_mentions_are_literal_observations_not_role_assignments,
        test_role_words_use_word_boundaries,
        test_root_index_records_co_mentions_without_inference,
        test_function_overlap_is_reference_only,
        test_mention_timeline_is_dated_and_deterministic,
    ]
    for test in tests:
        test()
    print(f'PUBLIC STATEMENT ROLE MENTION TESTS PASSED ({len(tests)} tests)')
    return 0
