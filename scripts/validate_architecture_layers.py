#!/usr/bin/env python3
"""Validate synchronized architecture, entanglement, terrain and Hawkins layers."""
...
def main():
    backend=load("data/backend.json"); manifest=load("data/atlas-manifest.json"); workflow=load("data/project-workflow.json"); tree=load("data/tree.json"); ent=load("data/entanglement.json"); terrain=load("data/axis-topology.json"); hawkins=load("data/hawkins-scale.json")
    required_paths={"data/backend.json","data/atlas-manifest.json","data/project-workflow.json","data/tree.json","data/entanglement.json","data/axis-topology.json","data/hawkins-scale.json","hawkins.html"}
    for rel in required_paths:
        if not(ROOT/rel).exists():ERRORS.append(f"Missing synchronized architecture path: {rel}")
    endpoints=backend.get("endpoints",{})
    for key,rel in {"entanglement":"data/entanglement.json","axisTopology":"data/axis-topology.json","hawkinsScale":"data/hawkins-scale.json","tree":"data/tree.json","projectWorkflow":"data/project-workflow.json","atlasManifest":"data/atlas-manifest.json"}.items():
    ...
    manifest_files={x for layer in manifest.get("layers",{}).values() if isinstance(layer,dict) for x in layer.get("files",[])}
    for rel in ("data/entanglement.json","data/axis-topology.json","data/hawkins-scale.json","hawkins.html"):
        if rel not in manifest_files:ERRORS.append(f"Atlas manifest does not register {rel}")
    expected_dimensions={"strength","dependency","distance","feedback","topology","trajectory","phase"}
    if not expected_dimensions.issubset(set(manifest.get("record_model",{}).get("entanglement",[]))):ERRORS.append("Atlas record_model entanglement fields are incomplete")
    rel_ext=ent.get("relationship_record_extension",{}); ent_required={"source","target","relationship","evidence","confidence","date","notes"}
    ...
    if not {"strength","distance","dependency","feedback","topology","trajectory","phase"}.issubset(set(rel_ext.get("recommended_entanglement_fields",[]))):ERRORS.append("Entanglement recommended fields are incomplete")
    require(ent.get("material_layer",{}).get("quantum_entanglement"),"entanglement.material_layer.quantum_entanglement");require(ent.get("material_layer",{}).get("network_entanglement"),"entanglement.material_layer.network_entanglement");require(ent.get("topology_model",{}).get("concepts"),"entanglement.topology_model.concepts");require(ent.get("trajectory_model",{}).get("fields"),"entanglement.trajectory_model.fields")
    ...
    for phase in ("emotion-state","comparative","calculations","website","release-audit"):
        if phase not in workflow_ids:ERRORS.append(f"Workflow omits required phase: {phase}")
    if "entanglement_principle" not in workflow.get("project_introduction",{}):ERRORS.append("Workflow mission lacks entanglement_principle")
    if "G9" not in {g.get("id") for g in workflow.get("gates",[]) if isinstance(g,dict)}:ERRORS.append("Workflow lacks entanglement evidence gate G9")
    print(f"Architecture layers checked: {len(required_paths)} required paths");print(f"Entanglement dimensions: {len(ent.get('relational_dimensions',[]))}");print(f"Terrain nodes: {len(tree_terrain)}");print(f"Hawkins levels: {len(levels) if isinstance(levels,list) else 0}");print(f"Workflow phases: {len(workflow.get('phases',[]))}");print(f"Errors: {len(ERRORS)} · Warnings: {len(WARNINGS)}")
