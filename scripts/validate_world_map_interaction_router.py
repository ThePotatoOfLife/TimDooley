...
def main() -> int:
    errors: list[str] = []
    for path in (ROUTER, LIFECYCLE, SUBDIVISIONS, BOOTSTRAP, APP, HANDOFF, HOVER, PLACES, COUNTRY, SPATIAL, GATEWAYS, INFRASTRUCTURE, IMPACT_ACTIONS, AXIS, TEST, COMPAT_TEST):
        if not path.exists():
    ...
    if not node:
        ...
        for path in (ROUTER, SUBDIVISIONS, APP, HANDOFF, HOVER, PLACES, COUNTRY, SPATIAL, GATEWAYS, INFRASTRUCTURE, IMPACT_ACTIONS, AXIS):
    ...
    print("- country, spatial overlays, Axis, capital, Gateway and Infrastructure interaction owned by the router on normal boots")
