...
const storyDir=new URL('../story-content/',import.meta.url);
const storyFragments=fs.readdirSync(storyDir).filter(name=>name.endsWith('.html')).map(name=>fs.readFileSync(new URL(name,storyDir),'utf8')).join('\n');
assert.ok(storyFragments.includes('Hear the full story'), 'Story fragment corpus must preserve its content expander');
...
assertLongformPage(tim,{name:'tim overview',host:'id="tim-tts"',css:'href="../app/tts-drawer.css"',reader:'src="../app/tts-reader.js"',drawer:'src="../app/tts-drawer.js"',adapter:'src="../app/longform-tts-adapter.js"',root:'data-tts-root=".tim-page"',item:'data-tts-item=".riddle,.long-section"',allLabel:'data-tts-all-label="Whole portrait"',currentLabel:'data-tts-current-label="Current section"',exclude:'data-tts-exclude="#tim-tts,.page-nav,.routes-footer,.thresholds,.route-line,.identity-row .meta,.question-doors,.evidence-class,.small-note,[data-house-topology-context],[data-no-tts]"'});
