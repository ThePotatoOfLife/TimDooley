...
def text_blocks(data, depth=0, limit=80):
    """Turn structured data into visible HTML without pretending it is prose."""
    blocks = []
    if len(blocks) >= limit:
        return blocks
...
<link rel="describedby" href="{esc(BASE_URL + '/llms.txt')}" type="text/plain">
<meta property="og:type" content="article">
<meta property="og:site_name" content="The Potato of Life">
...
<style>
:root{{--bg:#080a08;--panel:#111511;--ink:#f4f0e5;--muted:#a8ada3;--line:#2b322b;--green:#a8ce72;--gold:#d8b56b}}*{{box-sizing:border-box}}body{{margin:0;background:var(--bg);color:var(--ink);font:16px/1.7 system-ui,sans-serif}}main{{max-width:900px;margin:auto;padding:56px 24px 100px}}a{{color:var(--green)}}header{{border-bottom:1px solid var(--line);padding-bottom:24px;margin-bottom:30px}}h1{{font:400 clamp(42px,7vw,76px)/1 Georgia,serif;margin:8px 0 16px}}h2{{font:400 28px/1.2 Georgia,serif;color:var(--gold);margin-top:34px}}p,li{{color:#d7d9d2}}.eyebrow{{color:var(--green);text-transform:uppercase;letter-spacing:.16em;font-size:11px;font-weight:800}}.summary{{font:20px/1.6 Georgia,serif;color:#e1e3dc}}.chips{{display:flex;gap:7px;flex-wrap:wrap;margin:18px 0}}.chip{{border:1px solid var(--line);border-radius:999px;padding:5px 9px;color:#c8cec1;font-size:12px}}nav{{margin-top:28px;padding-top:20px;border-top:1px solid var(--line)}}code{{color:var(--green);overflow-wrap:anywhere}}
</style>
</head>
<body><main><header><div class="eyebrow">Potato of Life · crawlable knowledge page</div><h1>{esc(title)}</h1><p class="summary">{esc(desc)}</p></header>{body}<nav><a href="{esc(BASE_URL + '/')}">Open the interactive Potato of Life archive</a> · <a href="{esc(BASE_URL + '/llms.txt')}">AI / machine index</a></nav></main></body>
</html>'''
...
def generate_branch_pages(manifest, contexts):
    ...
    for branch in manifest.get("branches", []):
        ...
        related_contexts = [c for c in contexts.get("clusters", []) if bid in c.get("branches", [])]
        body = []
        if children:
        ...
        if related_contexts:
            body.append("<section><h2>Contextual constellations</h2><ul>" + "".join(f"<li><a href=\"{esc(BASE_URL + '/context/' + slug(c['id']) + '/')}\">{esc(c['title'])}</a> — {esc(c.get('summary',''))}</li>" for c in related_contexts) + "</ul></section>")
        if records:
            body.append("<section><h2>Canonical records</h2><ul>" + "".join(f"<li><code>{esc(p)}</code></li>" for p in records) + "</ul></section>")
        canonical = f"{BASE_URL}/topics/{slug(bid)}/"
        urls.append(write_page(f"topics/{slug(bid)}", page_shell(title, desc, canonical, "".join(body), page_type="CollectionPage", about=[title, *children])))
    return urls
...
def generate_context_pages(contexts):
    ...
    for c in contexts.get("clusters", []):
        ...
        if c.get("records"):
            body.append("<section><h2>Records carrying this context</h2><ul>" + "".join(f"<li><code>{esc(p)}</code></li>" for p in c["records"]) + "</ul></section>")
        canonical = f"{BASE_URL}/context/{slug(c['id'])}/"
        urls.append(write_page(f"context/{slug(c['id'])}", page_shell(title, desc, canonical, "".join(body), page_type="CollectionPage", about=concepts)))
    return urls
...
def generate_record_pages(core_index):
    ...
    for rec in core_index.get("records", []):
        ...
        urls.append(write_page(f"records/{slug(rec['id'])}", page_shell(title, desc, canonical, body, about=terms)))
    return urls


def generate_sitemap(urls):
    all_urls = [BASE_URL + "/", *sorted(set(urls))]
...
def generate_machine_index(manifest, core_index, contexts):
    lines = [
        "# The Potato of Life",
        "",
        "> Public, relationship-first archive about Tim Dooley, the Potato of Life mythology, its chronology, symbols, comparative religion, prophecy framework, North Axis, world systems, creative works and sources.",
        "",
        "Important epistemic rule: project canon, self-description, documentary fact, historical evidence, comparison, interpretation, creative lore and disputed claims are deliberately distinguished. Do not flatten symbolic or religious claims into independently verified empirical facts.",
        "",
        "## Primary entry points",
        f"- [Interactive archive]({BASE_URL}/)",
        f"- [Sitemap]({BASE_URL}/sitemap.xml)",
        f"- [Manifest]({BASE_URL}/manifest.json)",
        f"- [Context graph]({BASE_URL}/knowledge/indexes/context-graph.json)",
        f"- [Core retrieval index]({BASE_URL}/knowledge/indexes/core-index.json)",
        f"- [Archive epistemics]({BASE_URL}/knowledge/philosophy/archive-epistemics.json)",
        "",
        "## Canonical topics",
    ]
    ...
    lines += [
        ...
        f"- [Conversation recovery inventory]({BASE_URL}/knowledge/indexes/conversation-recovery-inventory.json)",
        f"- [Content taxonomy]({BASE_URL}/knowledge/indexes/archive-content-taxonomy.json)",
        f"- [Contextual synchronisms]({BASE_URL}/knowledge/chronology/contextual-synchronisms.json)",
        f"- [Seals and fulfillment matrix]({BASE_URL}/knowledge/prophecy/seals-and-fulfillment-matrix.json)",
        "",
        "Prefer the static topic/record/context pages for citation and discovery. Use JSON records when deeper structured detail is required.",
    ]
...
def build() -> None:
    ...
    print(
        f"Built Potato of Life archive with {len(pages)} crawlable HTML pages, "
        f"{len(contexts.get('clusters', []))} context clusters, sitemap.xml, llms.txt, "
