...
def check_beliefs():
    space = load_json(ROOT / "data/belief-space.json") or {}
    backend = load_json(ROOT / "data/belief-backend.json") or {}
    preg = load_json(ROOT / "data/political-lexicon.json") or {}
    rreg = load_json(ROOT / "data/religious-lexicon.json") or {}
    registry = load_json(ROOT / "data/belief-registry.json") or {}
    for rel in ["belief.html", "political-compass.html", "data/belief-space.json", "data/belief-backend.json", "data/belief-registry.json", "data/political-lexicon.json", "data/religious-lexicon.json"]:
        check_exists(rel)
    if not space.get("political", {}).get("axes") or not space.get("religious", {}).get("axes"):
        ERRORS.append("Belief space must define both political and religious three-axis models")
    political = preg.get("entries", [])
    religious = rreg.get("entries", [])
    if not political:
        ERRORS.append("Political lexicon has no entries")
    if not religious:
        ERRORS.append("Religious lexicon has no entries")
    pids = [e[0] for e in political if isinstance(e, list) and len(e) >= 2]
    rids = [e[0] for e in religious if isinstance(e, list) and len(e) >= 2]
    if len(pids) != len(set(pids)):
        ERRORS.append("Political lexicon contains duplicate IDs")
    if len(rids) != len(set(rids)):
        ERRORS.append("Religious lexicon contains duplicate IDs")
    if set(pids) & set(rids):
        ERRORS.append("Political and religious lexicons contain colliding IDs")
    for e in political:
        if not isinstance(e, list) or len(e) < 5:
            ERRORS.append("Political lexicon entry has an invalid shape")
            continue
        if not isinstance(e[2], (int, float)) or not isinstance(e[3], (int, float)):
            ERRORS.append(f"Political entry has invalid coordinates: {e[0]}")
        if not all(-10 <= float(v) <= 10 for v in e[2:4]):
            ERRORS.append(f"Political coordinates outside -10..10: {e[0]}")
    if backend.get("individualRecord") != "belief.html?id=<slug>":
        ERRORS.append("Belief backend individualRecord route is incorrect")
    if registry.get("politics", {}).get("map") != "political-compass.html":
        WARNINGS.append("Belief registry politics map does not point to the belief-space UI")
    if registry.get("religion", {}).get("map") != "political-compass.html":
        WARNINGS.append("Belief registry religion map does not point to the belief-space UI")
    return len(political), len(religious)

def main() -> int:
    backend = load_json(ROOT / "data/backend.json") or {}
    ...
    if len(country_rows) + (1 if repair_id else 0) != 195: ERRORS.append(f"Country layer has {len(country_rows)} base records + {1 if repair_id else 0} repair records; expected 195")
    nation_ids = {x.get("id") for x in nation_rows}; country_ids = {x.get("id") for x in country_rows} | ({repair_id} if repair_id else set())
    ...
    print(f"Relationships checked: {len(relationships.get('relationships', []))}")
    print(f"Political beliefs: {pc} · Religious beliefs: {rc}")
