#!/usr/bin/env python3
"""Generate the compact empirical data runtime consumed by the ordinary World Map.

This projection never fetches the network. Canonical country records and explicit
institutional membership data remain the source of truth; this file only reshapes
supported, comparable values for fast browser use.
"""
...
METRICS = {
    "gdp_per_capita": {
        ...
        "observation_aliases": ["gdp_per_capita"],
        "fallback": ("economy", "gdp_per_capita_usd"),
        "fallback_period": ("economy", "gdp_year"),
        "fallback_source": ("economy", "source"),
        "fallback_source_url": ("economy", "source_url"),
    },
    "real_growth": {
        "label": "Real GDP growth",
        "unit": "percent/year",
        "observation_aliases": ["real_growth", "real_gdp_growth"],
        "fallback": ("economy", "gdp_growth_percent"),
        "fallback_period": ("economy", "gdp_year"),
        "fallback_source": ("economy", "source"),
        "fallback_source_url": ("economy", "source_url"),
    },
    "inflation": {
        "label": "Inflation",
        "unit": "percent/year",
        "observation_aliases": ["inflation"],
        "fallback": ("economy", "inflation_percent"),
        "fallback_period": ("economy", "gdp_year"),
        "fallback_source": ("economy", "source"),
        "fallback_source_url": ("economy", "source_url"),
    },
    "unemployment": {
        ...
        "observation_aliases": ["unemployment"],
        "fallback": ("economy", "unemployment_percent"),
        "fallback_period": ("economy", "gdp_year"),
        "fallback_source": ("economy", "source"),
        "fallback_source_url": ("economy", "source_url"),
    },
    "debt_to_gdp": {
        "label": "Debt / GDP",
        "unit": "percent",
        "observation_aliases": ["debt_to_gdp", "government_debt_to_gdp"],
        "fallback": ("public_finance", "debt_to_gdp"),
        "fallback_period": ("public_finance", "year"),
        "fallback_source": ("public_finance", "source"),
        "fallback_source_url": ("public_finance", "source_url"),
    },
}
...
def fallback_cell(record: dict, metric_id: str, definition: dict):
    ...
    if isinstance(raw, dict):
        ...
        value = number(raw)
        period = nested(record, definition["fallback_period"])
        source = nested(record, definition["fallback_source"])
        source_url = nested(record, definition["fallback_source_url"])
    if value is None:
        return None
    return {
        "value": value,
        "unit": definition["unit"],
        "period": period,
        "source": source or "canonical country record",
...
def build_runtime() -> dict:
    ...
    for metric_id, definition in METRICS.items():
        ...
        metric_meta[metric_id] = {
            ...
            "unit": definition["unit"],
            "coverage": coverage[metric_id],
            "country_count": len(countries),
            "coverage_percent": round((coverage[metric_id] / len(countries)) * 100, 1),
            "period_min": observed_periods[0] if observed_periods else None,
            "period_max": observed_periods[-1] if observed_periods else None,
...
if __name__ == "__main__":
    ...
    print(json.dumps({
        "country_count": runtime["country_count"],
        "groups": {key: value["member_count"] for key, value in runtime["groups"].items()},
        "metrics": {key: value["coverage"] for key, value in runtime["metrics"].items()},
        "output": str(RUNTIME_OUT.relative_to(ROOT)),
    }, indent=2))
