...
def explicit_builds(record: dict) -> list[dict]:
    ...
    for key, raw in (record.get("observations") or {}).items():
        if not isinstance(raw, dict) or not BUILD_KEY.search(str(key)) or not raw.get("source") or not raw.get("source_url"): continue
...
def build_runtime_file(path: Path = RUNTIME_OUT) -> dict:
    runtime = build_runtime(); path.parent.mkdir(parents=True, exist_ok=True); path.write_text(json.dumps(runtime, ensure_ascii=False, separators=(",", ":")) + "\n", encoding="utf-8"); return runtime
