...
TRINITY_CANONICAL = BASE_URL + "/religion/trinity/"
...
def build_machine_files(entries, families):
    ...
    discovery = {
        ...
        "entrypoints": {
            "tim": TIM_CANONICAL, "potato_trinity": TRINITY_CANONICAL, "religion": BASE_URL + "/religion/", "philosophy": BASE_URL + "/philosophy/", "science": BASE_URL + "/science/", "world": BASE_URL + "/world/", "world_map": BASE_URL + "/world-map/",
    ...
    full += ["", "## Mature Potato of Life orientation", "- **Potato of Life** is the mature triune whole.", "- **Father** is source-facing; Tim Dooley occupies the mature Father/source-facing Potato identity.", "- **Son / Thomas / Vessel** is manifestation- and passage-facing and specializes through Door/Vessel.", "- **Spirit** is living continuity/flow rather than a third spatial field or competing biography.", "- Geometry remains two-field: Source Field and Manifestation Field overlap in Door; theological roles are not identical to those fields.", f"- Trinity reader: {TRINITY_CANONICAL}", f"- Canonical owner: {BASE_URL}/knowledge/theology/potato-of-life-trinity.json", "", "## Canonical question URLs"]
    for entry in entries:
        eid = slug(entry.get("id", entry.get("question", "")))
