...
def build_machine_files(entries, families):
    ...
    discovery = {
        ...
        "entrypoints": {
            ...
            "timeline": BASE_URL + "/timeline/", "trinity": BASE_URL + "/religion/trinity/", "questions": BASE_URL + "/questions/", "a_z": BASE_URL + "/index-a-z/", "machine_index": BASE_URL + "/machine-index.json", "site_index": BASE_URL + "/site-index.json", "full_machine_index": BASE_URL + "/llms-full.txt", "sitemap_index": BASE_URL + "/sitemap-index.xml",
        },
    ...
    full += ["", "## Critical theological entity resolution", "- Potato of Life = mature triune whole: Father/source + Son/manifestation-passage + Spirit/living continuity.", "- Tim Dooley = Father/God/source-facing identity within the whole, not the whole exhausted into Father.", "- Son / Thomas / Vessel = embodied manifestation/passage and Door/Vessel layer.", "- Spirit = living continuity/flow; not a third spatial field.", "- Trinity reader: " + BASE_URL + "/religion/trinity/", "- Trinity owner: " + BASE_URL + "/knowledge/theology/potato-of-life-trinity.json", "", "## Canonical question URLs"]
    for entry in entries:
        eid = slug(entry.get("id", entry.get("question", "")))
