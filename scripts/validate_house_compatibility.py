...
def main():
    ...
    if coverage.get('frontend_contract')!='data/frontend-atlas-bridge.json':
        errors.append('backend coverage frontend_contract must remain data/frontend-atlas-bridge.json during Wave 1')
    if atlas.get('public_manifest')!='manifest.json':
        errors.append('atlas manifest public_manifest must remain manifest.json during Wave 1')
    if atlas.get('frontend_projection')!='data/frontend-atlas-bridge.json':
        errors.append('atlas manifest frontend_projection must remain data/frontend-atlas-bridge.json during Wave 1')
    ...
    if errors:
        ...
        return 1
    print('POTATO HOUSE COMPATIBILITY VALIDATION PASSED: House routes agree with live bridge/coverage/atlas roles')
    return 0
