...
def main() -> int:
    errors: list[str] = []
    for path in (ROUTER, URL_BRIDGE, TEST, URL_TEST, CONSUMER_TEST, LIFECYCLE, BOOTSTRAP, PLACES, SUBDIVISIONS):
        if not path.exists():
    ...
    for token in (
        "function encodeInspectorPath",
        "function decodeInspectorPath",
        "function deriveInspectorPathFromUrl",
        "function createInspectorUrlBridge",
        "potato-atlas-inspector-change",
        "searchParams.set('inspect'",
        "window.__potatoAtlasInspectorUrl",
    ):
    ...
    if not node:
        ...
        for path in (ROUTER, URL_BRIDGE, PLACES, SUBDIVISIONS):
