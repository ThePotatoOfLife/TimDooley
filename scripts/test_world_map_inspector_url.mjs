...
import {
  encodeInspectorPath,
  decodeInspectorPath,
  deriveInspectorPathFromUrl,
  createInspectorUrlBridge,
} from '../world-map/3d-inspector-url.js';
...
assert.deepEqual(
  decodeInspectorPath(encoded).map(node => ({type:node.type,id:node.id,parent:node.parent})),
  [
...
assert.deepEqual(decodeInspectorPath('country:DNK/bogus:x'), [], 'invalid typed paths must fail closed');

const legacy = new URL('https://example.test/world-map/?country=DNK&subdivision=DK-83&place=place%3Ahaderslev');
assert.equal(deriveInspectorPathFromUrl(legacy), encoded, 'legacy domain params must project to one typed inspector path');
...
const bridge = createInspectorUrlBridge({
  getHref:() => href,
  replace:url => { href = String(url); replaced.push(href); },
  eventTarget,
});
...
listeners.get('potato-atlas-inspector-change')?.({
  detail:{stack:stack.slice(0,2), current:stack[1]},
});
hydrated = new URL(href);
assert.equal(hydrated.searchParams.get('inspect'), 'country:DNK/subdivision:DK-83');
assert.equal(hydrated.searchParams.get('country'), 'DNK');
assert.equal(hydrated.searchParams.get('subdivision'), 'DK-83');
assert.equal(hydrated.searchParams.has('place'), false, 'moving back to a subdivision must clear stale place URL state');

listeners.get('potato-atlas-inspector-change')?.({
  detail:{stack:[stack[0]], current:stack[0]},
});
hydrated = new URL(href);
