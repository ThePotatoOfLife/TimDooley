...
def require(text:str,marker:str,owner:str)->None:
    if marker not in text:raise SystemExit(f'ATLAS PAGE FAILED: {owner} missing {marker}')
def forbid(text:str,marker:str,owner:str)->None:
    if marker in text:raise SystemExit(f'ATLAS PAGE FAILED: {owner} contains forbidden {marker}')

def main()->int:
    model=build_runtime();nodes=by_id(model);root_id=model['root_id']
