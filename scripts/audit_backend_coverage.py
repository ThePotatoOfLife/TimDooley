...
def main():
    ...
    known_graph_ids=bridge_ids|registry_ids|node_ids; relationship_only=sorted(i for i in endpoint_files if i not in known_graph_ids and i not in {"<id>","<country-id>","<slug>"})
    report={"version":"1.1.0","files_scanned":len(files),"registered_or_covered_files":sum(1 for p in file_paths if matches_pattern(p,coverage_patterns) or p in registered),"unmapped_files":missing_owners,"invalid_json":invalid_json,"duplicate_ids":{k:sorted(set(v)) for k,v in sorted(duplicate_ids.items())},"exact_duplicate_file_contents":exact_duplicates,"relationship_only_unresolved_ids":relationship_only,"bridge_explicit_ids":len(bridge_ids),"graph_registry_ids":len(registry_ids),"node_ids":len(node_ids),"notes":["Duplicate IDs are candidates, not automatic deletion targets: overlays and research expansions can legitimately repeat an ID when inheritance is explicit.","Exact duplicate files are candidates for consolidation after consumer migration.","Unresolved relationship endpoints are the strongest orphan signal and should either be promoted, bridged or removed.","Schema and source files can remain backend-only when their owner and consumer are explicit.","Invalid JSON is reported with line/column/context so the audit artifact survives the failure and can drive the next repair."]}
