...
COVERAGE=DATA/"backend-coverage-map.json"; BRIDGE=DATA/"global-graph-bridge.json"; MANIFEST=DATA/"atlas-manifest.json"; BACKEND=DATA/"backend.json"
...
def relationship_endpoints(value):
    if isinstance(value,dict):
        source=value.get("source") or value.get("from") or value.get("subject"); target=value.get("target") or value.get("to") or value.get("object")
...
def main():
    ...
    coverage_patterns=[x["file"] for x in coverage.get("layers",[])]; registered=collect_registered_files(manifest)|collect_registered_files(backend)|set(coverage_patterns)
    files=iter_json_files(); file_paths={p.relative_to(ROOT).as_posix() for p in files}; missing_owners=sorted(p for p in file_paths if not matches_pattern(p,coverage_patterns) and p not in registered)
    ...
    duplicate_ids={k:v for k,v in id_locations.items() if len(set(v))>1}; exact_duplicates={k:sorted(v) for k,v in content_hashes.items() if len(v)>1}
    bridge_ids={x.get("id") for x in bridge.get("explicit_bridges",[]) if isinstance(x,dict)}; registry_ids=set(); graph_registry=DATA/"graph-registry.json"
    ...
    # A relationship endpoint is resolved when the repository declares that ID
    # anywhere in valid backend data. The bridge/registry/node sets remain useful
    # diagnostics, but they are not the only legitimate ownership sources.
    ...
    report={"version":"1.2.0","files_scanned":len(files),"registered_or_covered_files":sum(1 for p in file_paths if matches_pattern(p,coverage_patterns) or p in registered),"unmapped_files":missing_owners,"invalid_json":invalid_json,"duplicate_ids":{k:sorted(set(v)) for k,v in sorted(duplicate_ids.items())},"exact_duplicate_file_contents":exact_duplicates,"relationship_only_unresolved_ids":relationship_only,"bridge_explicit_ids":len(bridge_ids),"graph_registry_ids":len(registry_ids),"node_ids":len(node_ids),"declared_ids":len(declared_ids),"notes":["Duplicate IDs are candidates, not automatic deletion targets: overlays and research expansions can legitimately repeat an ID when inheritance is explicit.","Exact duplicate files are candidates for consolidation after consumer migration.","Unresolved relationship endpoints are the strongest orphan signal and should either be promoted, bridged or removed.","A relationship endpoint is considered resolved when its ID is declared by valid backend data; the bridge/registry/node counts are retained as diagnostics.","Schema and source files can remain backend-only when their owner and consumer are explicit.","Invalid JSON is reported with line/column/context so the audit artifact survives the failure and can drive the next repair."]}
    (DATA/"backend-coverage-report.json").write_text(json.dumps(report,indent=2,ensure_ascii=False)+"\n",encoding="utf-8")
    print(json.dumps(report,indent=2,ensure_ascii=False)); critical=bool(invalid_json) or bool(duplicate_ids) or bool(relationship_only) or bool(missing_owners)
