...
def explicit_value(obj,keys):
    ...
    return ''
def classify_repository(source,obj,rid,rtype,role,fam):
    ...
    if root in ROOTS and layer in (SPIRIT|MIND|MATTER) and ((root=='spirit' and layer in SPIRIT) or (root=='mind' and layer in MIND) or (root=='matter' and layer in MATTER)): return root,layer,scale if scale in SCALES else layer
    s=source.lower(); n=Path(s).name
    if fam=='potatoism': return 'spirit','meaning','record' if any(x in n for x in ('observation','research','source','corpus')) else 'concept'
    if fam=='religion-texts': return 'spirit','myths' if any(x in n for x in ('myth','edda','norse','saga')) else 'belief','record'
    ...
    if role=='relationship' or 'relationships' in s or 'graph' in n:return 'matter','network','network'
...
def walk(v,path,source,out):
    if isinstance(v,dict):
        ...
        if ident and (name or v.get('type') or v.get('kind') or v.get('category')):
            role=classify_role(source,v); rtype=str(v.get('type') or v.get('kind') or v.get('category') or '').strip().lower(); fam=family(source); root,layer,scale=classify_repository(source,v,ident,rtype,role,fam)
            out.append({'id':ident,'name':name or ident,'description':first(v,DESC_KEYS)[:1000],'type':rtype,'layer':str(v.get('layer') or ''),'record_role':role,'owner_family':fam,'repository_root':root,'repository_layer':layer,'repository_scale':scale,'classification_basis':'explicit' if explicit_value(v,('repository_root','root_branch','branch')) else 'source-family/type','source':source,'path':path})
