...
def main():
    errors=[]
    for path in (CONTRACT,RUNTIME,EVIDENCE,PLACES,TEST):
    ...
    for token in ("window.__potatoAtlasFreshness","function describe(","unknown-vintage","latest-available","historical-snapshot"):
    ...
    if node:
        for path in (RUNTIME,EVIDENCE,PLACES):
