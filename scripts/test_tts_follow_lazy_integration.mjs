...
assert.ok(greatBook.includes('detail.waitUntil(loadAllSlots())'), 'whole-book TTS must await lazy chapters');
...
assert.ok(source.includes("const follow=button('Follow reading','🎯')"), 'shared TTS drawer must contain the bullseye toggle');
assert.ok(source.includes('let followReading=saved.followReading===true'), 'follow-reading choice must persist');
...
assert.ok(source.includes('const prepareSection=typeof options.prepareSection'), 'drawer must retain async preparation support');
assert.ok(source.includes('await prepareSection(sectionId)'), 'speech must wait for lazy content before starting');
assert.ok(source.includes("typeof follow==='boolean'?follow:persistedFollow()"), 'page highlighting must honor the same persisted follow setting');
assert.ok(source.includes("mark?.scrollIntoView?.({block:followReading?'center':'nearest',inline:'nearest'})"), 'expanded reader must center the active word only when follow is enabled');
