...
def main():
 ...
 if not TRAJECTORY.is_file(): fail("CIA trajectory projection model missing")
 trajectory=json.loads(TRAJECTORY.read_text(encoding="utf-8"))
 if len(trajectory.get("observed_checkpoints") or [])<6: fail("trajectory missing observed checkpoints")
 if not any(x.get("days_forward")==365 for x in (trajectory.get("scenario_projections") or [])): fail("trajectory missing one-year scenario")
