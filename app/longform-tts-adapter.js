(function(root,factory){
  ...
  function buildLongformPayload(input={}){
    ...
    const sections=[];
    const whole=cleanText(input.whole),current=cleanText(input.current),selection=cleanText(input.selection);
  ...
  function selectionInside(doc,container){
    ...
    const range=selection.getRangeAt(0),node=range.commonAncestorContainer,element=node.nodeType===1?node:node.parentNode;
  ...
  async function requestContentPreparation(container,sectionId,EventCtor=root?.CustomEvent,options={}){
    ...
    const event=new EventCtor('potato:tts-prepare',{
      ...
      detail:{
        ...
        waitUntil(task){if(task&&typeof task.then==='function')pending.push(Promise.resolve(task));},
      },
    });
  ...
  function configFromElement(host,doc){
    ...
    const data=host.dataset||{},rootSelector=data.ttsRoot||'',container=rootSelector?doc.querySelector(rootSelector):null;
    if(!container)return null;
    return {mount:host,root:container,id:data.ttsId||container.id||'longform',label:data.ttsLabel||doc.title||'Read aloud',allLabel:data.ttsAllLabel||'Whole content',currentLabel:data.ttsCurrentLabel||'Current section',selectionLabel:data.ttsSelectionLabel||'Selection',itemSelector:data.ttsItem||'',excludeSelector:data.ttsExclude||''};
  }

  function mount(config={}){
    const doc=config.document||root?.document,Drawer=config.drawer||root?.PotatoTTSDrawer,host=config.mount,container=config.root;
    ...
    let currentItem=null,drawer=null,selectionAction=null,preparing=false;
    ...
    const source=()=>{
      ...
      return buildLongformPayload({id:config.id||container.id||'longform',label:config.label||doc.title||'Read aloud',allLabel:config.allLabel||'Whole story',currentLabel:config.currentLabel||'Current entry',selectionLabel:config.selectionLabel||'Selection',whole:readableText(container,config.excludeSelector||''),current:readableText(currentItem,config.excludeSelector||''),selection:selectionInside(doc,container)});
    };
    ...
    const setReadingActive=active=>{clearReadingActive();if(active&&currentItem?.classList)currentItem.classList.add('ptts-reading-active');};
    const chooseCurrent=item=>{
      setReadingActive(false);pageHighlighter.invalidate();
      if(item&&container.contains(item))currentItem=item;
      refresh();
    };
    const prepareForPlayback=async(sectionId,options={})=>{
      preparing=true;
      try{await requestContentPreparation(container,sectionId,doc.defaultView?.CustomEvent||root?.CustomEvent,options)}finally{
        preparing=false;pageHighlighter.invalidate();
        if(currentItem&&!container.contains(currentItem)){setReadingActive(false);currentItem=null}
        ensureListenButtons();
      }
      const next=source();drawer?.setPayload?.(next);return next;
    };

    drawer=Drawer.mount({
      target:host,getPayload:source,prepareSection:prepareForPlayback,settingsKey:config.settingsKey||'potato-tts-settings',
      onEvent:event=>{
    ...
    function ensureListenButtons(){
      ...
      for(const item of getItems()){
        ...
        const control=doc.createElement('button');control.type='button';control.className='ptts-inline-listen';control.dataset.ttsListen='';control.textContent='🔊 Listen';control.setAttribute('aria-label','Listen to this section');
        control.addEventListener('click',event=>{event.preventDefault();event.stopPropagation();chooseCurrent(item);drawer.playSection?.('current')});
        const anchor=item.querySelector?.('h2,h3,h1')||item.querySelector?.('.movement-label,.story-meta');
        if(anchor?.insertAdjacentElement)anchor.insertAdjacentElement('afterend',control);else if(item.prepend)item.prepend(control);else item.append?.(control);
    ...
    const onActivate=event=>{
      ...
      const item=event.target?.closest?.(itemSelector);if(item&&container.contains(item))chooseCurrent(item);
    };
    ...
    container.addEventListener('click',onActivate);container.addEventListener('focusin',onActivate);container.addEventListener('potato:tts-current',onCurrent);
    ...
    const observer=Observer?new Observer(records=>{
      ...
      pageHighlighter.invalidate();if(preparing)return;
      if(currentItem&&!container.contains(currentItem)){setReadingActive(false);currentItem=null}
      ensureListenButtons();refresh();
    }):null;
    observer?.observe(container,{childList:true,subtree:true,characterData:true});

    ensureListenButtons();refresh();selectionAction=Drawer.mountSelectionAction?.({document:doc,container,drawer,getPayload:source});

    return {drawer,refresh,setCurrent:chooseCurrent,source,ensureListenButtons,selectionAction,pageHighlighter,destroy(){observer?.disconnect();container.removeEventListener('click',onActivate);container.removeEventListener('focusin',onActivate);container.removeEventListener('potato:tts-current',onCurrent);selectionAction?.destroy?.();pageHighlighter.clear();clearReadingActive();drawer.stop?.()}};
  }
  ...
  function autoMount(doc=root?.document){
    ...
    for(const host of doc.querySelectorAll('[data-tts-longform]')){
      ...
      const config=configFromElement(host,doc);if(!config)continue;
      const instance=mount(config);if(instance){if(host.dataset)host.dataset.ttsMounted='true';mounted.push(instance)}
    }
    return mounted;
  }

  if(root?.document){const start=()=>autoMount(root.document);root.document.readyState==='loading'?root.document.addEventListener('DOMContentLoaded',start,{once:true}):start()}
