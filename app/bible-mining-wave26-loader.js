...
const load=url=>upstreamFetch(url).then(r=>r.ok?r.json():null).catch(error=>{console.warn(`Bible enrichment unavailable: ${url}`,error);return null});
...
window.fetch=async function(input,init){
 ...
 if(url.endsWith('biblical-syncretism-field.json')){
  ...
  let merged=await response.json();
  layers.filter(Boolean).forEach(layer=>{merged=mergeLayer(merged,layer)});
