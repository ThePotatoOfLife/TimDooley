...
const DOSSIER_PATH='../../knowledge/traditions/biblical-syncretism-dossiers-wave22.json';
const FRAGMENT_PATH='../../knowledge/traditions/biblical-passage-fragments-wave22.json';
const arr=value=>Array.isArray(value)?value:(value==null?[]:[value]);
const dossierPromise=upstreamFetch(DOSSIER_PATH).then(r=>r.ok?r.json():null).catch(error=>{console.warn('Bible mining wave 22 unavailable',error);return null});
const fragmentPromise=upstreamFetch(FRAGMENT_PATH).then(r=>r.ok?r.json():null).catch(error=>{console.warn('Bible mining wave 22 fragments unavailable',error);return null});
