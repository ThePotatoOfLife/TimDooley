...
async function loadHn(force){return cached('hn:new',async()=>{const ids=await fetchJson('https://hacker-news.firebaseio.com/v0/newstories.json');const rows=await Promise.allSettled((ids||[]).slice(0,45).map(id=>fetchJson('https://hacker-news.firebaseio.com/v0/item/'+id+'.json',6500)));return rows.filter(x=>x.status==='fulfilled').map(x=>normalizeHn(x.value)).filter(Boolean)},force)}
...
function storyCard(r,lead=false){
 ...
 return'<article class="news-story'+(lead?' news-story--lead':'')+'">'+image+'<div class="news-story-kicker"><span>'+esc(r.provider)+'</span><i></i><span>'+esc(kind)+'</span></div>'+(lead?'<h2>':'<h3>')+esc(r.title)+(lead?'</h2>':'</h3>')+summary+'<div class="news-story-footer"><span>'+esc(r.source||'source')+'</span>'+country+'<span>'+esc(relativeTime(r.published))+'</span><a href="'+esc(r.url)+'" target="_blank" rel="noopener noreferrer">Full report ↗</a></div></article>';
}
