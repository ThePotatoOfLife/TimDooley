...
async function bootFull(host,config){
 ...
 async function run(){
   const cat=catRow(),lr=lensRow(),q=custom||mergeQueries(cat.gdelt_query,lr?.gdelt_query),baseProviders=custom?['gdelt']:[...new Set([...(cat.providers||['gdelt']),...(lr?.providers||[])])],providers=(custom||lr)?baseProviders.filter(id=>id!=='publisher-rss'&&id!=='official-rss') : baseProviders,ms=horizonMs(config,horizon),timespan=horizonSpan(config,horizon);
