...
function buildJesusRows(atlas,prisonJesus){
 ...
 return sequence.map((entry,index)=>{
  ...
  return {
   ...
   evidence_kind:evidenceKinds.join(' + ')||'project chronology / later comparative synthesis',source_refs:unique([SOURCE_PATHS.jesusAtlas,...(contexts.length?[SOURCE_PATHS.prisonJesus]:[])]),
   chronology_context:contexts,prison_pairings:prisonPairings,jesus_context:unique(prisonPairings.map(pairing=>pairing.biblical_function)).join(' ')
  };
 });
}
...
function buildPassionRows(atlas,prisonJesus){
 ...
 return overlaps.map((item,index)=>{
  ...
  return {
   ...
   people:extractPeople(`${item.son||''} ${contexts.map(event=>event.project_record||'').join(' ')}`),evidence_kind:evidence.join(' + ')||'comparative inference',source_refs:unique([SOURCE_PATHS.passionAtlas,...(contexts.length?[SOURCE_PATHS.prisonJesus]:[])]),chronology_context:contexts
  };
 });
}
...
function rowSearchText(row,context){
 const key=dateKey(row.date||row.timestamp),matchedFragments=fragmentsFor(row,context.fragments),exact=exactEvidenceForSearch(row,context.occurrenceById),sameDate=context.occurrenceByDate.get(key)||[],thoughts=context.thoughtByDate.get(key)||[],theology=context.theologyByDate.get(key)||[],chronology=asArray(row.chronology_context),pairings=asArray(row.prison_pairings),signs=context.signSequence.filter(item=>sharesYear(row.date,item.period));
 return [relationText(row),...exact,...sameDate.map(item=>item.quote),...matchedFragments.map(item=>`${item.reference} ${item.text}`),...thoughts.flatMap(item=>[item.thought_direction,...asArray(item.signals),item.research_question]),...theology.flatMap(item=>[item.stage,...asArray(item.signals),item.interpretation]),...chronology.flatMap(item=>[item.event,item.public_record,item.project_record,item.public_crosscheck,item.significance,item.boundary]),...pairings.flatMap(item=>[item.motif,item.biblical_function,item.project_parallel,item.strength,item.mismatch,item.boundary]),...signs.flatMap(item=>[item.role,...asArray(item.examples),item.interpretive_value])].join(' ').toLowerCase();
}
