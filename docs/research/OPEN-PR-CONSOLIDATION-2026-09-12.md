...
| #40 | Bible comparator completion | **partial recovery** | Recover richer Bible reader/filter/wave/argument behavior and ownership checks. Normalize Chronology links to Timeline and keep Religion broad. Do not restore a separate validation workflow. |
...
| #33 | Door / entanglement / constraints / fate modeling | **recover selectively** | Large additive science, chronology, tradition and timeline-pack corpus. Import unique records, but rename `*-atlas.*` objects by actual function and preserve empirical/symbolic firewalls. |
...
| #31 | shared reader design / runtime hardening | **partial recovery** | Recover missing runtime/path/retry/site-shell fixes only. Chronology page changes are obsolete because Timeline is canonical. |
...
| #29 | Christianity / Jesus / eschatology | **recover selectively** | Recover theology research corpus, denominational matrix, scholar map, Thomas faith chronology and FAQ. Rename retired Atlas files and subordinate public Christianity material to Religion rather than creating a competing top-level owner. |
...
| #25 | Great Book / prediction / 1987–2030 Timeline | **recover selectively** | Recover unique chronology/core/culture/FAQ records. Timeline UI itself must be reconciled with current canonical Timeline. Rename Chapter-24 Atlas record by function. |
...
- `/timeline/` is the only current chronology product.
