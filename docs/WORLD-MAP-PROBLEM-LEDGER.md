...
**Status:** in progress; Germany official-source acquisition is now freshness-guarded on branch `world-map-deu-freshness-guard-20260921`.  
...
**Progress continued:** the BKG contract now distinguishes the current 01.01.2026 direct-download GeoPackage from the separately published WFS, whose BKG service page still reports 01.01.2025. The 2026 GeoPackage is the only preferred source for current-vintage import; the WFS is explicitly reference-only while its vintage lags. CI now enforces preferred-source/vintage alignment and blocks stale-service substitution.  
**Remaining:** acquire/hash the 2026 GeoPackage bytes, extract/review `VG250_LAN`, generate/review the bounded DEU partition, build bounded DEU Places coverage from a reviewed fresh Places source, move the source contract to `ready-for-import`, then promote it without country-specific renderer code.
