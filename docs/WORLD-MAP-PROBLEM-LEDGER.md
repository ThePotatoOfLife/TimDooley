...
**Status:** in progress on branch `world-map-surface-tokens-current-20260921b`.  
...
**Progress:** semantic stacking bands are already on main. This wave adds shared menu/context/overlay/panel background tokens plus common border, radius, shadow and blur tokens, and migrates the shell, World Bar, country card, Evidence, Path, Impact and Chain surfaces while preserving semantic border/color distinctions.  
**Remaining:** only promote spacing/padding tokens where repetition is genuinely cross-surface; do not flatten specialized data/visual semantics.
