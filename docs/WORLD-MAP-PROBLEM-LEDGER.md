...
**Status:** in progress on branch `world-map-surface-tokens-20260921`.  
...
**Progress:** semantic stacking bands are now on main. This wave adds a small shared surface palette for menu/context/overlay/panel backgrounds plus common border, radius, shadow and blur tokens, and migrates the shell, World Bar, country card, Evidence, Path, Impact and Chain surfaces while preserving their special semantic border colors.  
**Remaining:** audit spacing/padding tokens only where repetition is genuinely cross-surface; do not flatten data-semantic colors or specialized visualization styling.
