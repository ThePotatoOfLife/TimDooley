...
- [~] Replace the GeoNames mirror seed with a reviewed fresh canonical build. Official `cities15000.zip` source/provenance semantics are now formalized; reviewed current dump acquisition and generated replacement remain.
...
- [~] Continue design-token convergence beyond z-index. Shared menu/context/overlay/panel surface roles now cover the primary map investigation surfaces; keep spacing/padding convergence selective and preserve semantic borders/data colors.
...
- [~] Consolidate global UI design tokens, z-index bands and common surfaces. Semantic stacking bands are on main; shared background/border/radius/shadow/blur roles now cover the primary menus, context cards and investigation overlays, with selective spacing convergence still remaining.
