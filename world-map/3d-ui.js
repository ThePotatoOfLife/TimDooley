...
function tuneMapSurface() {
  const map = window.__potatoAtlasMap;
  if (!map) return;
  // Canonical country paint channels are intentionally not tuned here.
  // Compositor owns country fill/extrusion color; Country Selection owns outlines;
  // Physical World owns country fill opacity. Progressive UI only styles auxiliary layers.
