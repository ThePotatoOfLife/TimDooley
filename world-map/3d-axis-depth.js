...
function breadthFor(data, dimension) {
  return Number(data.geometry_law?.normalized_breadth?.[String(dimension)] ?? .64);
}

function spiralPosition(data, index, total, level) {
  const centerX = 50;
  const top = 7;
  const spanY = 86;
  const y = top + (index / Math.max(total - 1, 1)) * spanY;
  const phase = index * Math.PI * 0.72;
  const breadth = breadthFor(data, level.dimension);
  const amplitude = 8 + breadth * 25;
  return {x:centerX + Math.sin(phase) * amplitude, y, breadth};
}
...
function directionLabel(current) {
  if (current.dimension > 5) return 'Ascending · narrower / more convergent';
  if (current.dimension === 5) return 'Door / first-heaven threshold';
  if (current.dimension === 4) return 'Ordinary world layer · real-world anchor';
...
function renderPanel(data, current) {
  ...
  const breadth = breadthFor(data,current.dimension);
  const sevenIndex = current.dimension >= 5 ? current.dimension - 4 : null;
  panel.innerHTML = `
    ...
    <div class="boundary"><b>Boundary:</b> D1–D11 is a spiritual/project-symbolic navigation grammar. D4 is ordinary geographic map space. D5–D11 form seven project upper stages, with external seven-heaven traditions used only as comparative neighbors. M-theory/Hermetic parallels are analogies, not physical or doctrinal identity claims.</div>
    <div class="grid">
      <div class="metric"><span>Dimension</span><b>D${current.dimension}</b><small>${esc(current.status || 'scaffold')}</small></div>
      <div class="metric"><span>Axis breadth</span><b>${Math.round(breadth*100)}%</b><small>${current.dimension<4?'broader / dispersed':current.dimension>4?'narrower / convergent':'world anchor'}</small></div>
      ${sevenIndex ? `<div class="metric"><span>Upper stage</span><b>${sevenIndex} / 7</b><small>D5–D11 project sequence</small></div>` : ''}
    ...
    <div class="card"><b>Matter ↕ Spirit geometry</b><div class="row">↑ narrower · convergence · integration · source-facing orientation</div><div class="row">● D4 · real-world geographic anchor</div><div class="row">↓ wider · manifestation · dispersion · disintegration risk</div><p class="muted">“Inner” here means convergence of represented relations, not merely private psychology. Width is symbolic and never territorial or physical size.</p></div>
    <div class="card"><b>Above / below reflection</b><p class="muted">The same event, memory, obligation or relationship can be inspected through upward and downward operators. This is functional correspondence rather than a one-to-one mirrored floor count.</p></div>
...
function installNavigator(map, data) {
  ...
  funnel.style.cssText = 'position:absolute;left:8%;right:8%;top:4px;height:388px;opacity:.22;pointer-events:none;background:linear-gradient(to bottom,rgba(223,248,255,.18),rgba(159,232,255,.12) 37%,rgba(132,149,141,.11) 47%,rgba(85,70,52,.16) 70%,rgba(48,38,27,.24));clip-path:polygon(41% 0,59% 0,64% 36%,70% 47%,78% 61%,90% 78%,100% 100%,0 100%,10% 78%,22% 61%,30% 47%,36% 36%)';
  track.appendChild(funnel);
  const curve = document.createElement('div');
  curve.style.cssText = 'position:absolute;left:50%;top:4px;width:2px;height:388px;background:linear-gradient(to bottom,#f1fbff,#9fe8ff 46%,#78a8a2 56%,#6e695f 68%,#33291f);transform:translateX(-50%);opacity:.56;border-radius:999px';
  track.appendChild(curve);

  levels.forEach((level,index)=>{
    const pos = spiralPosition(data,index,levels.length,level);
    const button = document.createElement('button');
    button.dataset.axisDimension = String(level.dimension);
    button.title = `${level.description} · symbolic breadth ${Math.round(pos.breadth*100)}%`;
    const short = level.label.replace(/^D\d+ · /,'').replace(' / First Heaven','').replace(' / Disintegration','').replace(' / World Map','');
    button.innerHTML = `<span style="display:block;font-size:10px;font-weight:800">D${level.dimension}</span><span style="display:block;font-size:8px;white-space:nowrap;max-width:92px;overflow:hidden;text-overflow:ellipsis">${esc(short)}</span>`;
