function createInspectorRouter(options = {}) {
  ...
  function snapshot(reason = 'change') {
    const detail = {
      reason,
      current:publicNode(stack.at(-1)),
      stack:stack.map(publicNode),
      depth:stack.length,
    };
    emit(detail);
    return detail;
  ...
  function setBaseline(node) {
    ...
    const changed = !currentBaseline
      || currentBaseline.type !== next.type
      || currentBaseline.id !== next.id
      || currentBaseline.owner !== next.owner;
