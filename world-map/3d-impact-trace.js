...
function ensurePanel() {
  ...
  style.textContent = `
    #atlasImpactContext{position:absolute;right:10px;bottom:10px;z-index:var(--atlas-z-overlay,8);width:min(370px,calc(100% - 20px));max-height:min(65vh,600px);overflow:auto;padding:11px 12px;background:var(--atlas-surface-overlay-bg,#080c0ced);border:1px solid #465754;border-radius:var(--atlas-surface-radius,11px);box-shadow:var(--atlas-surface-shadow,0 10px 28px #0009);backdrop-filter:var(--atlas-surface-blur,blur(11px))}#atlasImpactContext[hidden]{display:none!important}
