const INSPECTOR_TYPES = Object.freeze(['country','subdivision','place']);
...
function normalizePathNodes(nodes = []) {
  ...
  for (const raw of nodes) {
    ...
    const parent = normalized.length
      ? { type:normalized.at(-1).type, id:normalized.at(-1).id }
      : null;
...
function decodeInspectorPath(value = '') {
  ...
  try {
    for (const segment of text.split('/')) {
      ...
      if (split <= 0) return [];
      const type = segment.slice(0, split);
      const id = decodeURIComponent(segment.slice(split + 1));
      nodes.push({ type, id });
    }
  } catch {
    return [];
...
function createInspectorUrlBridge(options = {}) {
  ...
  function hydrate() {
    ...
    if (derived) {
      url.searchParams.set('inspect', derived);
      return commit(url, derived);
    }
    path = '';
    return path;
  }
  ...
  function state() {
    return { path, nodes:decodeInspectorPath(path) };
  }
  ...
  function destroy() {
    eventTarget?.removeEventListener?.('potato-atlas-inspector-change', syncFromInspector);
  }
