...
function installControl(map,cfg){
  ...
  const params=new URL(location.href).searchParams,allowed=['all','north','west','east','south','center','brics','off'];select.value=allowed.includes(params.get('field'))?params.get('field'):DEFAULT_VIEW;
  function apply(view){const visible=view!=='off';[FILL_ID,LINE_ID].forEach(id=>map.setLayoutProperty(id,'visibility',visible?'visible':'none'));if(visible){map.setFilter(FILL_ID,viewFilter(view));map.setFilter(LINE_ID,viewFilter(view));map.setPaintProperty(FILL_ID,'fill-color',viewColor(view,cfg));map.setPaintProperty(LINE_ID,'line-color',viewColor(view,cfg));map.setPaintProperty(FILL_ID,'fill-opacity',viewOpacity(view));}const url=new URL(location.href);if(view===DEFAULT_VIEW)url.searchParams.delete('field');else url.searchParams.set('field',view);history.replaceState(null,'',url);}
  select.addEventListener('change',()=>apply(select.value));apply(select.value);
}
...
function installInteractions(map){
  ...
  map.on('mouseenter',FILL_ID,e=>{map.getCanvas().style.cursor='pointer';const p=e.features?.[0]?.properties||{};const lines=FIELD_NAMES.filter(n=>p[n]===1).map(n=>`${n[0].toUpperCase()+n.slice(1)} · ${roleLabel(p[`${n}_role`])}`);const institutional=p.brics_member===1?'BRICS · member':p.brics_partner===1?'BRICS · partner':'';popup.setLngLat(e.lngLat).setHTML(`<div class="atlas-hover"><b>${esc(p.name||p.iso3||'Country')}</b><br>${lines.length?lines.map(esc).join('<br>'):'No project-field assignment'}${institutional?`<br><span>${esc(institutional)}</span>`:''}<br><small>Project fields are interpretive and confidence-weighted; Center is convergence; BRICS is empirical.</small></div>`).addTo(map);});
  map.on('mouseleave',FILL_ID,()=>{map.getCanvas().style.cursor='';popup.remove();});
}
async function boot(){const[cfgRes,geoRes]=await Promise.all([fetch(CFG_URL),fetch(GEO_URL)]);if(!cfgRes.ok||!geoRes.ok)throw new Error('Axis field data unavailable');const[cfg,geo]=await Promise.all([cfgRes.json(),geoRes.json()]);for(let i=0;i<120&&!window.__potatoAtlasMap;i++)await new Promise(r=>setTimeout(r,50));const map=window.__potatoAtlasMap;if(!map)return;if(!map.loaded())await new Promise(r=>map.once('load',r));addLayers(map,geo,cfg);installControl(map,cfg);installLegend(cfg);installInteractions(map);}
