// Country Pulse is the deep country-inspector summary for the World Relational Atlas.
// The selected-country card owns the ordinary Current Map answer. Pulse therefore
// concentrates on sourced statistics, systems, institutions and deeper context.
...
const INDICATOR_UNITS = {
  'SP.POP.TOTL':'people','NY.GDP.MKTP.CD':'current USD','NY.GDP.PCAP.CD':'current USD / person',
  'NY.GDP.MKTP.KD.ZG':'percent','FP.CPI.TOTL.ZG':'percent','SL.UEM.TOTL.ZS':'percent of labour force',
  'SP.DYN.LE00.IN':'years','SP.DYN.TFRT.IN':'births / woman','SP.URB.TOTL.IN.ZS':'percent of population',
};
...
function normalizeObservation(value, fallback = {}) {
  ...
  return {
    ...
    period:value.year ?? value.reference_period ?? value.period ?? fallback.period ?? null,
    source:value.source || fallback.source || null,
  };
}
function scalarMetric(value, unit, period, source) {
  const number = Number(value);
  return Number.isFinite(number) ? {value:number,unit,period,source} : null;
}
function metric(record, id, demoRow = null) {
  const observations = record?.observations || {};
  const aliases = {
    population:['population'], gdp:['gdp'], gdp_per_capita:['gdp_per_capita'],
    real_growth:['real_growth','real_gdp_growth'], inflation:['inflation'], unemployment:['unemployment'],
    debt_to_gdp:['debt_to_gdp','government_debt_to_gdp'], life_expectancy:['life_expectancy'],
    fertility:['fertility'], urbanization:['urbanization'],
  };
  ...
  if (id === 'population') return normalizeObservation(demoRow?.population) || scalarMetric(population.value,'people',population.year,population.source);
  if (id === 'gdp' && Number.isFinite(Number(economy.gdp_current_usd_trillion))) return scalarMetric(Number(economy.gdp_current_usd_trillion)*1e12,'current USD',economy.gdp_year,economy.source);
  if (id === 'gdp_per_capita') return scalarMetric(economy.gdp_per_capita_usd,'current USD / person',economy.gdp_year,economy.source);
  if (id === 'real_growth') return scalarMetric(economy.gdp_growth_percent ?? economy.real_growth_percent_2025,'percent',economy.gdp_year || 2025,economy.source);
  if (id === 'inflation') return scalarMetric(economy.inflation_percent,'percent',economy.gdp_year || record?.updated,economy.source);
  if (id === 'unemployment') return scalarMetric(economy.unemployment_percent,'percent',economy.gdp_year || record?.updated,economy.source);
  if (id === 'debt_to_gdp') return scalarMetric(record?.public_finance?.debt_to_gdp_percent,'percent of GDP',record?.public_finance?.year,record?.public_finance?.source);
  if (id === 'life_expectancy') return scalarMetric(population.life_expectancy,'years',population.year,population.source);
  if (id === 'fertility') return scalarMetric(population.fertility_rate,'births / woman',population.year,population.source);
  return null;
}
function compactNumber(value) {
  const n = Number(value);
  if (!Number.isFinite(n)) return '—';
  return new Intl.NumberFormat('en',{notation:Math.abs(n)>=1e6?'compact':'standard',maximumFractionDigits:Math.abs(n)>=100?1:2}).format(n);
}
function displayValue(item) {
  ...
  if (String(item.unit).startsWith('percent')) return `${compactNumber(item.value)}%`;
...
function listValues(value, max = 8) {
  if (!value) return [];
  if (Array.isArray(value)) return value.filter(item => typeof item === 'string' && item.trim()).slice(0,max);
  return typeof value === 'string' ? [value] : [];
}
function mergeLists(...values) { return [...new Set(values.flatMap(value => listValues(value)))].slice(0,8); }
function chips(values) { return values.length ? `<div class="pulse-chips">${values.map(value=>`<span>${esc(value)}</span>`).join('')}</div>` : '<div class="muted">No structured items in this record yet.</div>'; }

function religionSummary(row) {
  const composition = row?.religion?.composition || {};
  const items = Object.entries(composition).filter(([,value])=>Number.isFinite(Number(value))).sort((a,b)=>Number(b[1])-Number(a[1])).slice(0,3);
  if (!items.length) return '';
  return `<div class="pulse-religion"><b>Religion / identity · ${esc(row.religion.year || 2020)}</b>${items.map(([key,value])=>`<div class="pulse-religion-row"><span>${esc(title(key))}</span><strong>${Number(value).toFixed(Number(value)>=10?0:1)}%</strong></div>`).join('')}</div>`;
}
...
function moreStatistics(record, demoRow) {
  const preferred = [['life_expectancy','Life expectancy'],['fertility','Fertility'],['urbanization','Urbanization']];
  ...
  for (const [key,raw] of Object.entries(record?.observations || {})) {
    ...
    const value = normalizeObservation(raw); const label = title(key);
    if (!value || seen.has(label.toLowerCase())) continue;
    rows.push([label,value]); seen.add(label.toLowerCase());
  }
...
function pulseHtml(code, record, demoRow) {
  ...
  const systems = mergeLists(record?.energy_and_resources?.strategic_assets,record?.energy_and_resources?.observed_system_features,record?.trade_and_value_chains?.strategic_value_chains,record?.infrastructure?.strategic_assets);
  const projects = mergeLists(record?.projects,record?.programmes,record?.research_queue,record?.energy_and_resources?.projects).slice(0,6);
  const institutions = mergeLists(record?.political_system?.legislature,record?.security_and_external_relations?.major_organizations,record?.security_and_external_relations?.alliances);
  ...
  return `<section class="card atlas-country-pulse" data-pulse-code="${esc(code)}">
    ...
    <div class="pulse-depth-note">Inspector depth · sourced country baseline beyond the map card</div>
...
async function render({force=false}={}) {
  ...
  try {
    ...
    if (!force && existing?.dataset.pulseCode === code && lastRenderCode === code) return;
    ...
    const holder = document.createElement('div'); holder.innerHTML = pulseHtml(code,record,demo?.countries?.[code]);
    const pulse = holder.firstElementChild; const firstGrid = panel.querySelector('.grid');
    if (pulse && firstGrid?.parentNode) firstGrid.parentNode.insertBefore(pulse,firstGrid); else if (pulse) panel.appendChild(pulse);
    lastRenderCode = code;
    ...
    window.dispatchEvent(new CustomEvent('potato-atlas-inspector-rendered',{detail:{code,surface:'country-pulse'}}));
  } finally {
    rendering = false;
    if (renderQueued) { renderQueued = false; queueMicrotask(()=>render({force:true})); }
  }
}
function ensureStyle() {
  if (document.getElementById('atlasCountryPulseStyle')) return;
  const style = document.createElement('style'); style.id = 'atlasCountryPulseStyle';
  style.textContent = `.atlas-country-pulse{border-color:#526258;background:#111a18}.pulse-heading{display:flex;align-items:flex-start;justify-content:space-between;gap:8px}.pulse-heading b{display:block;font:400 19px Georgia,serif;margin-top:2px}.pulse-depth-note{margin-top:7px;color:var(--muted);font-size:9px}.pulse-metrics{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:6px;margin-top:9px}.pulse-metric{padding:7px;border:1px solid var(--line);border-radius:8px;background:#0d1514}.pulse-metric span{display:block;color:var(--muted);font-size:9px;text-transform:uppercase}.pulse-metric strong{display:block;font-size:14px;margin-top:2px}.pulse-metric small,.pulse-stat-row small{display:block;color:var(--muted);font-size:9px;line-height:1.25;margin-top:2px}.pulse-religion{margin-top:9px;padding-top:8px;border-top:1px solid var(--line)}.pulse-religion-row{display:flex;justify-content:space-between;gap:8px;padding:3px 0}.pulse-section{border-top:1px solid var(--line);margin-top:8px;padding-top:6px}.pulse-section>summary{cursor:pointer;color:#dfe6dc;font-size:12px;font-weight:650}.pulse-connections{margin-top:5px}.pulse-connection{display:flex;width:100%;justify-content:space-between;gap:8px;align-items:center;text-align:left;margin:4px 0;padding:6px 7px}.pulse-connection small{color:var(--muted);text-align:right}.pulse-chips{display:flex;flex-wrap:wrap;gap:4px;margin-top:6px}.pulse-chips span{border:1px solid var(--line);border-radius:999px;padding:3px 6px;font-size:10px}.pulse-stat-list{margin-top:6px}.pulse-stat-row{padding:5px 0;border-bottom:1px solid var(--line)}.pulse-stat-row:last-child{border:0}.pulse-stat-row span{color:var(--muted);font-size:10px}.pulse-stat-row b{display:block}.pulse-quality{display:flex;justify-content:space-between;gap:7px;flex-wrap:wrap;margin-top:8px;padding-top:7px;border-top:1px solid var(--line);color:var(--muted);font-size:9px}`;
  document.head.appendChild(style);
}
ensureStyle();
panel?.addEventListener('click',event=>{const button=event.target.closest('[data-pulse-country]');if(button?.dataset.pulseCountry)window.goCountry?.(button.dataset.pulseCountry);});
...
window.addEventListener('potato-atlas-module-ready',event=>{if(event?.detail?.label==='Demography')render({force:true});});
window.__potatoAtlasCountryPulse={render,getRecord,metric,get currentCode(){return activeSelectionCode()||null;}};
render();
