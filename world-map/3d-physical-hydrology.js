...
function ensureSources() {
  if (!map.getSource(BASIN_SOURCE)) {
    map.addSource(BASIN_SOURCE, { type:'geojson', data:lastBasins, attribution:'HydroSHEDS · HydroBASINS · Lehner & Grill (2013)' });
  }
  if (!map.getSource(RIVER_SOURCE)) {
    map.addSource(RIVER_SOURCE, { type:'geojson', data:lastRivers, attribution:'HydroSHEDS · HydroRIVERS · Lehner & Grill (2013)' });
  }
}
...
function ensureLayers() {
  ...
  if (!map.getLayer(BASIN_FILL)) {
    map.addLayer({
      ...
      paint:{ 'fill-color':'#4b8392', 'fill-opacity':0.055 },
    }, beforeFill);
  }
  if (!map.getLayer(BASIN_LINE)) {
    map.addLayer({
      ...
      source:BASIN_SOURCE,
      layout:{ visibility:'none' },
      paint:{ 'line-color':'#69a5b3', 'line-opacity':0.52, 'line-width':['interpolate',['linear'],['zoom'],4,0.55,8,1.15] },
    }, beforeLine);
  }
  if (!map.getLayer(RIVER_LINE)) {
    map.addLayer({
      id:RIVER_LINE,
      type:'line',
      source:RIVER_SOURCE,
      ...
      paint:{
        'line-color':'#67b8da',
        'line-opacity':0.9,
        'line-width':['interpolate',['linear'],['zoom'],4,0.55,6,0.9,9,1.35],
      },
    }, beforeLine);
  }
}
...
function setVisibility(visibility) {
  for (const id of [BASIN_FILL, BASIN_LINE, RIVER_LINE]) {
...
async function refreshViewport() {
  ...
  setStatus(`Hydrology · loading regional drainage ≥ ${threshold.toLocaleString()} km² catchments`);
  ...
  setStatus(failures
    ? `Hydrology · partial regional data · ${basinCount} basins · ${riverCount} major river segments`
    : `Hydrology · ${basinCount} basins · ${riverCount} major river segments`);
}
...
async function enable() {
  ...
  enabled = true;
  setVisibility('visible');
  setStatus(map.getZoom() < MIN_ZOOM ? 'Hydrology · zoom in to regional scale' : 'Hydrology · loading regional drainage');
  scheduleRefresh();
  return true;
}
